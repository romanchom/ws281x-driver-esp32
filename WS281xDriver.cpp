...
static constexpr auto DIVIDER = 2;
static constexpr auto DURATION = 12.5; /* minimum time of a single RMT duration
				in nanoseconds based on clock */

static constexpr auto PULSE_T0H = static_cast<uint32_t>(300 / (DURATION * DIVIDER));
static constexpr auto PULSE_T0L = static_cast<uint32_t>(1300 / (DURATION * DIVIDER));
static constexpr auto PULSE_T1H = static_cast<uint32_t>(800 / (DURATION * DIVIDER));
static constexpr auto PULSE_T1L = static_cast<uint32_t>(1300 / (DURATION * DIVIDER));
static constexpr auto PULSE_TRS = static_cast<uint32_t>(50000 / (DURATION * DIVIDER));

static void IRAM_ATTR bytesToPulses(
